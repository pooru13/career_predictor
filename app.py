...
def login():
    if request.method == 'POST':
        ...
        # LOGIN FETCH
        cur.execute(
            "SELECT id, name FROM users WHERE email=? OR phone=?",
        ...
        if user:
            ...
            # 🔥 CHECK LAST CAREER
            cur.execute(
                "SELECT career FROM career_history WHERE user_id=? ORDER BY id DESC LIMIT 1",
                (user_id,)
            )
            last_career = cur.fetchone()

            conn.close()

            # ✅ CONDITION
            if last_career:
                career = last_career[0]
                session['career'] = career
                return redirect(f'/daily/{career}')   # 👈 DIRECT TASK PAGE

            else:
                return redirect('/predict')  # 👈 NEW USER
...
def daily_plan(career):
    ...
    # ✅ progress %
    progress_percent = int((day / total_tasks) * 100)
...
def predict():
    if request.method == 'POST':
        ...
        # ✅ SAVE CAREER IN SESSION
        session['career'] = career
        # after prediction, we save the career choice in the database for the user. This way, when they log in again, we can show them their last chosen career and direct them to the appropriate roadmap or daily tasks.
...
# 🔥 SAVE IN DB
        ...
        cur.execute(
    "INSERT INTO career_history (user_id, career) VALUES (?, ?)",
    ...
    return render_template(
        "index.html",
        name=session.get('name'),
        career=session.get('career')   # 👈 important
    )
